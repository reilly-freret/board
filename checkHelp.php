...
$stmt = $mysqli->prepare("DELETE FROM help WHERE session=? ORDER BY id ASC LIMIT 1");
if(!$stmt){
...
$stmt = $mysqli->prepare("select phoneNumber FROM help WHERE session=? ORDER BY id ASC LIMIT 1");
if(!$stmt){
	printf("Query Prep Failed: %s\n", $mysqli->error);
	exit;
}
